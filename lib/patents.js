...
function parseTWPATResults(html) {
  ...
  $('td.sumtd2_PN').each((_, pnCell) => {
    ...
    // Extract abstract (摘要)
    const abstract = row.find('td.sumtd2_AB').first().text().trim().replace(/^\uFEFF/, '');
...
// Three-step TIPO fetch: init session → search → expand display columns → paginate
async function fetchTIPOExpanded(keyword, existingIds = new Set(), timeoutMs = 15000) {
  const MAX_PAGES = 5; // max 50 results per keyword
  ...
  const cookies1 = searchHtml.cookies || '';
  ...
  // Step 3: collect page 1, then paginate
  const allPatents = parseTWPATResults(expandHtml);

  // Check if page 1 already all-cached — stop early
  const page1AllCached = allPatents.every(p => existingIds.has(`tipo-${p.patentNumber}`));
  if (page1AllCached && allPatents.length > 0) return allPatents;

  let currentHtml = expandHtml;
  let pageCookies = cookies1;

  for (let page = 2; page <= MAX_PAGES; page++) {
    const curActionMatch = currentHtml.match(/action="([^"]+)"/);
    const curInfoMatch = currentHtml.match(/name=INFO value=([0-9a-fA-F]+)/);
    if (!curActionMatch || !curInfoMatch) break;

    await delay(800);

    const nextHtml = await postForm(
      'tiponet.tipo.gov.tw',
      curActionMatch[1],
      { 'INFO': curInfoMatch[1], 'BUTTON': 'GO', 'JPAGE': String(page) },
      timeoutMs,
      false,
      pageCookies,
    );

    const pagePatents = parseTWPATResults(nextHtml);
    if (!pagePatents.length) break;

    allPatents.push(...pagePatents);

    // Stop if all results on this page are already cached
    const allCached = pagePatents.every(p => existingIds.has(`tipo-${p.patentNumber}`));
    if (allCached) break;
...
async function fetchTIPO(existingIds) {
  ...
  for (const keyword of SEARCH_KEYWORDS) {
    try {
      const patents = await fetchTIPOExpanded(keyword, existingIds);
...
const INSIGHT_SYSTEM = `你是台灣教育科技產業情報分析師。
...
async function generateInsightOne(p) {
  const title = p.title.split('\n')[0].trim();
  const user = `標題：${title}\n專利類型：${p.patentType || '發明'}\n發明人：${(p.inventors || []).join('、')}\n摘要：${(p.abstract || '').substring(0, 500)}`;
  const raw = await callGroq(INSIGHT_SYSTEM, user, { temperature: 0.4, maxTokens: 700 });
  const match = raw.match(/\{[\s\S]*\}/);
  if (!match) throw new Error('No JSON in response');
  const obj = JSON.parse(match[0]);
  if (!obj.competitor || !obj.maturity || !obj.trend || !obj.moat) throw new Error('Missing insight fields');
  return obj;
}

async function generateInsights(patents) {
  for (const p of patents) {
    if (!p.abstract && !p.title) continue;
    try {
      p.insight = await generateInsightOne(p);
    } catch (err) {
      console.error('Insight generation error:', err.message);
    }
    await delay(2500); // stay under Groq rate limit
  }
}
...
async function refreshPatents() {
  ...
  // If TIPO returned nothing (blocked or no new patents), preserve existing cache
  // to avoid resetting updatedAt and causing the next scheduled run to skip.
  if (newItems.length === 0 && existing.length > 0) {
    console.log('[patents] No new items from TIPO, preserving existing cache');
    return cached;
  }
