...
const KEYWORD_CATEGORIES = {
  '學習': ['數位學習', '學習平台', '智慧教育', '學習系統'],
  '教學': ['教育科技', '教學平台', '教學系統'],
  '考試評測': ['線上測驗', '線上評量', '自動批改', '出題系統'],
  '校務': ['校務系統'],
};
...
async function readCache() {
  // Try Blob first on Vercel, then fall back to bundled file
  if (isVercel()) {
...
function parseTWPATResults(html) {
  ...
  $('td.sumtd2_PN').each((_, pnCell) => {
    ...
    const status = row.find('td.sumtd2_LS').text().trim();
    ...
    const inventors = inventorsRaw
      ? inventorsRaw.split(/[;\n]/)
          .map(s => s.replace(/\(.*?\)/g, '').trim())      // remove (國家)
          .filter(s => /[\u4e00-\u9fff]/.test(s))           // keep only entries with CJK chars
          .map(s => s.replace(/[A-Z,.\-\s]+$/g, '').trim()) // strip trailing English
          .filter(s => s.length > 0)
      : [];
...
// Two-step TIPO fetch: search → expand display columns (發明人, 摘要, 公報)
async function fetchTIPOExpanded(keyword, timeoutMs = 15000) {
  // Step 1: keyword search
  const searchData = {
    'INFO': '0074FE440000000000000000000003',
    'BUTTON': '檢索',
    '@_5_5_T': 'T_XX',
    '_5_5_T': keyword,
    '@_5_6_K': 'K_DATETYPE',
  ...
  const searchHtml = await postForm(
    'tiponet.tipo.gov.tw',
    '/twpat3/twpatc/twpatkm?@@1674640309',
    searchData,
    timeoutMs,
    true, // return cookies
  );
...
module.exports = { getPatents, refreshPatents };
