...
async function searchNews(query, maxResults = 20) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    const results = await fetchBingNews(query, maxResults);
    if (results.length > 0) return results;
    // Retry once after a short delay (Bing cold start can return empty)
    if (attempt < MAX_RETRIES - 1) await new Promise(r => setTimeout(r, 1000));
  }
  return [];
}

async function fetchBingNews(query, maxResults) {
  ...
  let parsed;
  try {
    // strict:false tolerates malformed XML from Bing but uppercases all keys
    parsed = await parseStringPromise(xml, { explicitArray: false, strict: false });
  } catch {
    return [];
  }

  const channel = (parsed.RSS || parsed.rss) && (parsed.RSS || parsed.rss).CHANNEL || (parsed.rss && parsed.rss.channel);
  if (!channel) return [];
  const rawItems = channel.ITEM || channel.item;
  if (!rawItems) return [];
  ...
  return items
    .map(item => {
      const bingLink = item.LINK || item.link || '';
      const realUrl = extractRealUrl(bingLink);
      const source = (item['NEWS:SOURCE'] || item['news:source'] || '').trim();
      ...
      return {
        ...
        source,
        pubDate,
        snippet: (item.DESCRIPTION || item.description || '').replace(/<[^>]*>/g, '').trim(),
      };
    })
    .filter(a => {
